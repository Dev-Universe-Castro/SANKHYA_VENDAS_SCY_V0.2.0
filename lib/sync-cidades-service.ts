        ...
        const PAYLOAD = {
            "requestBody": {
                "dataSet": {
                    ...
                    "entity": {
                        "fieldset": {
                            "list": "CODCID, UFNOMECID, CODREG, DESCRICAOCORREIO, NOMECID"
                        }
                    }
                }
            }
    ...
    for (let i = 0; i < cidades.length; i += BATCH_SIZE) {
        ...
        for (const cidade of batch) {
         ...
         WHEN NOT MATCHED THEN
           INSERT (
             ID_SISTEMA, CODCID, UFNOMECID, CODREG, DESCRICAOCORREIO, NOMECID,
             SANKHYA_ATUAL, DT_ULT_CARGA, DT_CRIACAO
           )
           VALUES (
             :idSistema, :codCid, :ufNomeCid, :codReg, :descricaoCorreio, :nomeCid,
             'S', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
           )`,
                {
                    idSistema,
                    codCid: cidade.CODCID || null,
                    ufNomeCid: cidade.UFNOMECID || null,
